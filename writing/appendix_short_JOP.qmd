...
![Per-class precision, recall, and F1 on the held-out test set.](figures/training/fig7_classification_report.png){#fig-train_class_report fig-align="center" width="500"}

![Confusion matrix on the held-out test set (row-normalized recall).](figures/training/fig9_confusion_normalized.png){#fig-train_confusion_norm fig-align="center" width="500"}
...
country_langs <- lan %>%
  mutate(entry = paste0("**", Name, "**: ", Language)) %>%
  pull(entry)

n <- length(country_langs)
n_rows <- ceiling(n / 3)

# Pad so all columns have the same number of rows
country_langs <- c(country_langs, rep("", n_rows * 3 - n))

three_col <- data.frame(
  ` ` = country_langs[1:n_rows],
  `  ` = country_langs[(n_rows + 1):(2 * n_rows)],
  `   ` = country_langs[(2 * n_rows + 1):(3 * n_rows)],
  check.names = FALSE
)
...
# Define the Subsaharan African countries
...
![Commodities on which Russia is import-reliant](import_reliance_heading.png){#fig-import_reliance_heading fig-align="center" width="50%" fig-pos="H"}
...
![Courtries exporting commodities on which Russia is import-reliant](import_reliance_heading_exporters.png){#fig-import_reliance_heading_exporters fig-align="center" width="50%" fig-pos="H"}
...
![Commodities on which Russia is export-reliant](export_reliance_heading.png){#fig-export_reliance_heading fig-align="center" width="50%" fig-pos="H"}
...
![Countries import-reliant on commodities on which Russia is export-reliant](export_reliance_heading_importers.png){#fig-export_reliance_heading_importers fig-align="center" width="50%" fig-pos="H"}
